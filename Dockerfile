...
RUN chmod 0755 /etc/glassfish/docker-entrypoint.sh && \
    chown -R glassfish:glassfish $GLASSFISH_HOME /etc/glassfish /etc/glassfish.d
...
ENTRYPOINT ["/etc/glassfish/docker-entrypoint.sh"]
